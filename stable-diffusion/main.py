...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        default=[
            "/home/chrishsieh/dev/DLCV/textual_inversion/configs/stable-diffusion/fine.yaml"
        ],
    )
    parser.add_argument(
        "-t",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        default="/home/jay/Project/dlcv-fall-2024-hw2-JayYang920110/2-3/stable-diffusion/ldm/models/stable-diffusion-v1/model.ckpt",
...
if __name__ == "__main__":
    ...
    try:
        ...
        trainer_config["find_unused_parameters"] = False
        ...
        if opt.placeholder_string:
            config.model.params.personalization_config.params.placeholder_strings = [
                opt.placeholder_string
            ]

        if opt.init_word:
            config.model.params.personalization_config.params.initializer_words[0] = (
                opt.init_word
            )
        ...
        if not cpu:
            if isinstance(gpus, int):
                ngpu = gpus
            # 如果 gpus 是字符串类型，执行字符串处理
            elif isinstance(gpus, str):
                ngpu = len(gpus.strip(",").split(","))
            else:
                raise ValueError(f"Unexpected gpus type: {type(gpus)}")
        ...
        print(f"Total time: {end - start} seconds")  # 输出总运行时间
...
#         default=[
#             "/home/chrishsieh/dev/DLCV/textual_inversion/configs/stable-diffusion/v1-finetune.yaml"
#         ],
#     )
#     parser.add_argument(
#         "-t",
