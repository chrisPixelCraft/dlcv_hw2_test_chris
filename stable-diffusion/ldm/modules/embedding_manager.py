...
#         for placeholder_string, placeholder_token in self.string_to_token_dict.items():
#             # Ensure placeholder_token is a tensor and on the correct device
#             placeholder_token = torch.tensor(placeholder_token, device=device)

#             placeholder_embedding = self.string_to_param_dict[placeholder_string].to(
#                 device
#             )

#             # Use torch.eq for safer tensor comparison
#             placeholder_idx = torch.where(torch.eq(tokenized_text, placeholder_token))
...
#                     placeholder_embedding.shape[0], max_step_tokens
#                 )
#                 placeholder_rows, placeholder_cols = placeholder_idx
