...
class DDPM(pl.LightningModule):
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        # Add this before loading state dict
        if hasattr(self.cond_stage_model, "transformer") and hasattr(
            self.cond_stage_model.transformer.text_model.embeddings.token_embedding, "weight"
        ):
            # Get the old and new vocab sizes
            old_vocab_size = sd["cond_stage_model.transformer.text_model.embeddings.token_embedding.weight"].shape[0]
            new_vocab_size = self.cond_stage_model.transformer.text_model.embeddings.token_embedding.weight.shape[0]
            
            if new_vocab_size != old_vocab_size:
                # Resize the token embedding weights to match new vocabulary size
                old_embedding_weight = sd["cond_stage_model.transformer.text_model.embeddings.token_embedding.weight"]
                new_embedding_weight = torch.nn.functional.pad(
                    old_embedding_weight, 
                    (0, 0, 0, new_vocab_size - old_vocab_size)
                )
                sd["cond_stage_model.transformer.text_model.embeddings.token_embedding.weight"] = new_embedding_weight
